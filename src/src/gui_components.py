...
class ParametersPanel(ttk.Frame):
    ...
    def load_config(self):
        ...
        # Use path_resolver to get config directory
...
class SetupPanel(ttk.Frame):
    ...
    def load_sample_data(self):
        ...
        image_extensions = {'.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff'}
        image_files = [
            f for f in os.listdir(sample_dir_str)
            if os.path.isfile(os.path.join(sample_dir_str, f)) and
            Path(f).suffix.lower() in image_extensions
        ]
        ...
        if not image_files:
            messagebox.showwarning(
                ...
                f"Found files: {os.listdir(sample_dir_str)}"
            )
            return

        # Load the sample directory
        self.fragment_folder = sample_dir_str
        self.folder_label.config(text=sample_dir_str, foreground="black")
        self._load_fragments()
    ...
    def _run_pipeline_thread(self, variant, params):
        ...
        try:
            ...
            # Ensure output directories exist (path_resolver already creates them)
    ...
    def load_results(self):
        ...
        try:
            ...
            for name in ['fragment_contours.png', 'compatibility_heatmap.png', 'convergence.png']:
                ...
                if filepath.exists():
                    other_files.append(str(filepath))
            ...
            if not all_image_files:
                self.placeholder_label.config(
                    text=f"No result images found in {output_dir}\n\n"
