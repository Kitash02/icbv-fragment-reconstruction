"""
Variant 2: Hierarchical Ensemble Post-Processing (Fast Routing)

Changes from baseline:
- Replace ensemble_verdict_five_way() with ensemble_verdict_hierarchical()
- Uses hierarchical decision tree for fast routing
- Fast rejection: morphological check first
- Fast match: color + geometric consensus
- Full voting: only for hard cases (~20% of pairs)
- Description: "Hierarchical routing for speed without accuracy loss"
...
from ensemble_voting import ensemble_verdict_hierarchical  # KEY CHANGE
...
def reclassify_borderline_cases(
    ...
    """
    Re-classify WEAK_MATCH pairs using HIERARCHICAL ensemble voting (Variant 2).
    """
    ...
    logger.info("=" * 60)
    logger.info("HIERARCHICAL ENSEMBLE POST-PROCESSING (Variant 2)")
    logger.info("=" * 60)
    ...
    for asm_idx, assembly in enumerate(assemblies):
        ...
        for pair in assembly['pairs']:
            ...
            # KEY CHANGE: Use hierarchical ensemble instead of five_way
            ensemble_verdict = ensemble_verdict_hierarchical(
    ...
    logger.info("=" * 60)
    logger.info("HIERARCHICAL ENSEMBLE SUMMARY (Variant 2)")
    logger.info("=" * 60)
