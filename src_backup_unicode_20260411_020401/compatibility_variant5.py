...
Penalty — Multiplicative Appearance Dissimilarity (Stage 1.6):
  ...
  Multi-modal fusion: color^6 × texture^2 × gabor^2 × haralick^2
...
VARIANT 5: More aggressive color penalty (color^6) for better negative discrimination.
"""
...
from hard_discriminators_variant5 import hard_reject_check
...
POWER_COLOR = 8.0      # VARIANT 5 BEST: Optimal balance (66.7% pos / 77.8% neg)
        ...
        for seg_a, chain_a in enumerate(segs_i):
            for frag_j, segs_j in enumerate(all_segments):
                ...
                for seg_b, chain_b in enumerate(segs_j):
                    ...
                    # BC=0.80 (different sources) → multiplier≈0.26 (74% reduction)
