"""
Hard Discriminator Module - Variant 0 Iteration 1
EVOLUTIONARY OPTIMIZATION: First tightening step

Iteration 1 Strategy:
- Baseline (iter0) showed only ~78% positive, ~75% negative (estimated)
- Need significant tightening to reduce false positives (cross-source matches)
- Incrementally raise thresholds: 0.70→0.72 color, 0.65→0.67 texture

Target: Push negative accuracy above 85% while maintaining positive above 70%

Changes from baseline (hard_discriminators.py):
- Line 125: bc_color < 0.70 → bc_color < 0.72
- Line 125: bc_texture < 0.65 → bc_texture < 0.67
"""
...
def hard_reject_check(
    ...
    # ITERATION 1: Raised from 0.70/0.65 to 0.72/0.67
    if bc_color < 0.72 or bc_texture < 0.67:
        logger.debug(
