"""
Variant 3: Tuned Weighted Ensemble Post-Processing

Changes from baseline:
- Uses weighted ensemble with custom tuned weights
- Color weight increased to 0.40 (from 0.35)
- Texture weight decreased to 0.15 (from 0.20)
- Emphasizes color discrimination over texture
"""
...
# Custom weights for Variant 3
CUSTOM_WEIGHTS = {
    'color': 0.40,          # Increased (primary discriminator)
    'raw_compat': 0.25,     # Unchanged
    'texture': 0.15,        # Decreased
    'morphological': 0.15,  # Unchanged
...
def reclassify_borderline_cases(
    ...
    """
    Re-classify WEAK_MATCH pairs using TUNED WEIGHTED ensemble voting (Variant 3).
    """
    ...
    logger.info("=" * 60)
    logger.info("TUNED WEIGHTED ENSEMBLE POST-PROCESSING (Variant 3)")
    logger.info("=" * 60)
    logger.info("Custom weights: color=0.40, raw=0.25, texture=0.15, morph=0.15, gabor=0.05")
    ...
    logger.info("=" * 60)
    logger.info("TUNED WEIGHTED ENSEMBLE SUMMARY (Variant 3)")
    logger.info("=" * 60)
