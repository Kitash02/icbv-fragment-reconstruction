"""
Variant 9A: Enhanced Ensemble Post-Processing
==============================================

Builds on Variant 9 with optimized weights for pottery discrimination.

Key Changes:
1. Color weight boosted to 0.45 (from 0.40) - pottery primary discriminator
2. Morphological reduced to 0.10 (from 0.15) - secondary feature
...
# Color is PRIMARY discriminator (pigment chemistry is artifact-specific)
CUSTOM_WEIGHTS = {
    'color': 0.45,          # INCREASED - primary discriminator
    'raw_compat': 0.25,     # Geometric features (unchanged)
    'texture': 0.15,        # Local patterns (unchanged)
    'morphological': 0.10,  # DECREASED - secondary feature
    'gabor': 0.05          # Frequency-domain (unchanged)
}
...
def reclassify_borderline_cases(
    ...
    """
    Re-classify WEAK_MATCH pairs using enhanced ensemble (Variant 9A).

    Enhanced features:
    - Optimized weights (color=0.45 primary)
    ...
    logger.info("=" * 60)
    logger.info("ENHANCED FULL RESEARCH STACK (Variant 9A)")
    logger.info("=" * 60)
    logger.info("Optimized weights: color=0.45, raw=0.25, texture=0.15, morph=0.10, gabor=0.05")
    ...
    logger.info("=" * 60)
    logger.info("ENHANCED ENSEMBLE SUMMARY (Variant 9A)")
    logger.info("=" * 60)
